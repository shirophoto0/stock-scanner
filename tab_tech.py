...
################################
    ...
    with st.sidebar.expander("⚙️ เมนูตัวกรองหุ้น", expanded=True):
        ...
        # 🔧 แก้บั๊ก: เดิม filtered_df ถูกสร้างขึ้นเฉพาะตอนมีข้อมูลหุ้นอยู่แล้วเท่านั้น (ในเงื่อนไข
        # if ด้านล่าง) แต่โค้ดจุดอื่นด้านล่างของฟังก์ชัน (ตารางผลลัพธ์การสแกน) ไปเรียกใช้ตัวแปรนี้
        # แบบไม่มีเงื่อนไข ถ้าตอนนั้นยังไม่เคยมีข้อมูลหุ้นเลย (เช่น กดอัปเดตครั้งแรกสุด) จะ error ทันที
        # ตั้งค่าเริ่มต้นไว้ก่อนเสมอ เพื่อให้จุดอื่นอ้างอิงได้อย่างปลอดภัยไม่ว่าจะมีข้อมูลหรือไม่ก็ตาม
        filtered_df = None
