...
def load_and_calculate_stock_data():
    ...
    def main():
        ...
        with tab_stock:
            ...
            # จัดการ Session State เพื่อเก็บชื่อหุ้น
    ...
    ###################################################################
        # 3. ส่วน TFEX (ต้องเช็คก่อนว่ามี tab_tfex หรือไม่)
        if tab_tfex is not None:
            # 2. ส่วน TFEX
            with tab_tfex:
                ...
                with sub_tfex_close:
                    ...
                    if not open_trades.empty:
                        ...
                        if st.button("ยืนยันการปิดสถานะ"):
                            ...
                            update_trade_close(st.session_state.current_sheet_id, selected_trade_id, close_price, str(close_date))
